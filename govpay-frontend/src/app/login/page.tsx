...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const result = await request<{ id: number; username: string; email: string; role: string; requiresTwoFactor: boolean; token?: string | null }>(
        "/api/Auth/login",
        {
          method: "POST",
      ...
      if (result.requiresTwoFactor) {
        window.localStorage.setItem("govpay_pending_username", username);
        ...
        <div className="-mt-10 rounded-[1.5rem] bg-white p-6 shadow-lg shadow-slate-200/80">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 px-4 py-3 font-semibold text-white shadow-lg shadow-emerald-500/20 transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-70"
            >
